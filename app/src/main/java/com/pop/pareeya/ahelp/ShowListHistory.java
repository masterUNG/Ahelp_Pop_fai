...
public class ShowListHistory extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            Log.d(tag, "strJSON1 ==> " + strJSON1);

            JSONArray jsonArray = new JSONArray(strJSON1);
            String[] Call_IDStrings = new String[jsonArray.length()];
            String[] Call_DataTime = new String[jsonArray.length()];
            String[] nameCall_IDStrings = new String[jsonArray.length()];

            for (int i=0;i<jsonArray.length();i++) {

                JSONObject jsonObject = jsonArray.getJSONObject(i);
                Call_IDStrings[i] = jsonObject.getString("Call_ID");
                Call_DataTime[i] = jsonObject.getString("Call_DataTime");
                nameCall_IDStrings[i] = findNameCall(Call_IDStrings[i], urlStrings[1]);

                Log.d(tag, "Name ==> " + nameCall_IDStrings[i]);
    ...
    private String findNameCall(String call_idString,
                                String urlPHP) {
